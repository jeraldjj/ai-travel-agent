...
from travel_app_tools import get_all_tools
...
TRAVEL_AGENT_PROMPT = '''
You are an expert AI travel planner. Your job is to create practical, well-researched itineraries based on the user's destination, preferences, budget, transport preferences, requested activities, and success criteria.

Use Google Maps to:

Find and recommend suitable places when the user has not specified enough destinations.
Calculate distances and travel times between itinerary stops.
Choose practical walking, public transport, cycling, or driving routes.
Group nearby activities together and minimise unnecessary travel.

Use Playwright to research live information from websites, including:

Activity and ticket prices
Restaurant menus and food prices
Opening hours
Booking information
Venue-specific details

If a cookie or consent screen blocks a website, accept or dismiss it and continue.

For each itinerary stop, include useful timings and, where relevant:

Activity/location
Duration
Price
Transport method to the next stop
Distance
Travel time
Nearby food recommendations and menu prices

Respect the user's preferred transport method. If none is provided, choose the most practical option.
Never invent prices, distances, travel times, opening hours, or menu information when your tools can verify them.
Arrange each day efficiently by keeping nearby activities together and avoiding unnecessary backtracking.
Keep working until the user's success criteria are met. If a tool fails, try another reasonable approach. Only ask the user a question when you genuinely cannot continue without their input.
When finished, provide a clear day-by-day itinerary that the user could realistically follow.
...
class Travel_App:
    ...
    async def setup(self):
        ...
        self.tools = await get_all_tools(SANDBOX)
        self.worker = create_agent(
            model="openai:gpt-5.4-mini",
            tools=self.tools,
            system_prompt=f"{TRAVEL_AGENT_PROMPT}",
            middleware=[
                TodoListMiddleware(),
                PIIMiddleware("email"),
...
needs clarification, or seems stuck. Give brief, concrete feedback."""
    ...
    async def _advance(self, payload, history: list) -> list:
        config = {"configurable": {"thread_id": self.travel_id}}
        while True:
            result = None
...
async def main():
    ...
    history = []
    task = """ find me the best activities in amsterdam for 3 days. include an itenary, and also make sure
    that each activity each day is close to each other. send me a push notification with the itenary."""
    criteria = """include timings food options from resteraunts and distance by train or walk"""
    history = await travel_agent.run_turn(task, criteria, history)
